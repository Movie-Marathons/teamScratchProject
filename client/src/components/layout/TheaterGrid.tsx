...
export default function TheaterGrid({ searchParams }: Props) {
  ...
  const handleViewMovies = (entry: TheaterEntry) => {
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        console.log("Loaded showtimes:", Array.isArray(films) ? films.length : films);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    const dateStr = searchParams.date ? new Date(searchParams.date).toISOString().split("T")[0] : undefined;
    ...
    fetch(url, { signal: controller.signal })
      ...
      .then((data) => {
        const list = Array.isArray(data) ? data : (data?.results ?? data?.cinemas ?? []);
        console.log("Loaded theaters from backend:", Array.isArray(list) ? list.length : list);
        console.log("Sample theater item:", list && list.length ? list[0] : null);
      ...
      .catch((err) => {
        ...
        console.error(`Failed to load theaters from ${url}: ${(err as Error).message}`, err);
        setError(`Sorry — we couldn't load theaters from ${url}. (${(err as Error).message})`);
      })
  ...
  if (loading) {
    return (
      <p className="text-center mt-10">Loading theaters…</p>
    );
  }
  ...
  const filtered = theaters.filter((entry) => {
    ...
    const sz = (entry?.cinema as any)?.search_zip ? String((entry.cinema as any).search_zip) : "";
    const zipMatches = (pc && pc === qzip) || (sz && sz === qzip) || (!pc && !sz); // if no zip data, don't exclude

    const dates = Array.isArray(entry?.cinema?.show_dates) ? entry.cinema.show_dates : [];
    const hasDates = dates.length > 0;
    const dateMatches = !selectedDateStr || !hasDates || dates.some((d) => d?.date === selectedDateStr);
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        {filtered.map((entry) => {
          ...
          return (
            <Card
              key={theater.cinema_id || `${theater.cinema_name}-${theater.postcode}`}
