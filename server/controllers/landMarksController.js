...
async function listLandmarks(req, res, next) {
  try {
    ...
    if (
      nums.length !== 4 ||
      nums.some((n) => Number.isNaN(n) || !Number.isFinite(n))
    ) {
      return res
        .status(400)
        .json({
          error: 'Bounding box parameters must be numeric and provided.',
        });
    }
    ...
    if (nums[0] >= nums[2] || nums[1] >= nums[3]) {
      return res
        ...
        .json({
          error:
            'Bounding box coordinates must satisfy west < east and south < north.',
        });
    }
    ...
    if (nums[2] - nums[0] > maxSpan || nums[3] - nums[1] > maxSpan) {
      return res
        .status(400)
        .json({ error: `Bounding box span must be <= ${maxSpan} degrees.` });
    }
...
async function listLandmarksByZip(req, res, next) {
  try {
    ...
    if (!zip || !/^\d{5}$/.test(zip)) {
      return res
        .status(400)
        .json({ error: 'Invalid or missing zip parameter' });
    }
    ...
    if (radiusMi !== undefined) {
      ...
      if (Number.isNaN(r) || r <= 0) {
        return res
          .status(400)
          .json({ error: 'radiusMi must be a positive number.' });
      }
